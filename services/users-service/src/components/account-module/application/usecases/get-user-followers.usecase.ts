...
export class GetUserFollowers {
  ...
  async execute({ id }: GetAccountByIdRequest): Promise<AccountResponse[]> {
    ...
    const follows = await this.followRepositoryPort.findUserFollowers(profile);
    return follows.map((follow) => AccountMapper.requestToGetResponseReversed(follow.follower));
  }
}
